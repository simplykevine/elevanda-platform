...
class UserManager(BaseUserManager):
    def create_user(self, email, password=None, **extra_fields):
        ...
        # SHA-512 hashing as required by spec, then Django hashes on top for storage
        sha512_hash = hashlib.sha512(password.encode()).hexdigest()
        user.set_password(sha512_hash)
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    is_verified = models.BooleanField(default=False)  # Admin must verify device first
